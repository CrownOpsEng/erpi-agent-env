...
install -m 0755 "$SELF_DIR/scripts/postgres-server-build.sh" "$WORK/postgres-server-build.sh"
IMAGE="$(build_docker_image_ref "$POSTGRES_BUILD_IMAGE" "$POSTGRES_BUILD_IMAGE_SHA256")"
build_run_logged "Rebuild qualified PostgreSQL/plpgsql_check payloads ($JOBS workers)" "$WORK/native-rebuild.log" \
  docker run --rm --network none \
    ...
    -e ERPI_BUILD_JOBS="$JOBS" \
    -e HOST_UID="$(id -u)" -e HOST_GID="$(id -g)" \
    -v "$WORK:/work" "$IMAGE" bash -lc '
      set -euo pipefail
      restore_owner() { chown -R "$HOST_UID:$HOST_GID" /work >/dev/null 2>&1 || true; }
      trap restore_owner EXIT
      /work/postgres-server-build.sh
      STAGE=/work/stage/usr/local/pg-build
      cd /work/plpgsql_check-2.8.11
      make -j"$ERPI_BUILD_JOBS" USE_PGXS=1 PG_CONFIG="$STAGE/bin/pg_config" >/dev/null
      make USE_PGXS=1 PG_CONFIG="$STAGE/bin/pg_config" DESTDIR=/work/pl-stage install >/dev/null
    '
STAGE="$WORK/stage/usr/local/pg-build"; mkdir -p "$WORK/client-payload/bin" "$WORK/client-payload/lib"
