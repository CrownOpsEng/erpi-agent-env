...
9. To publish, update `BUNDLE_VERSION`, require that exact source commit to pass **Accept runtime**, then use the permanent **Publish release** workflow. Do not invent a separate tag/release path; the publisher creates the matching tag and dispatches the release distribution build.
